...
(function() {
  ...
  let EventEmitter, buildCSOptionParser, exec, spawn,
    indexOf = [].indexOf;
...
If called without options, \`coffee\` will run your script.`;
  ...
  const SWITCHES = Object.freeze([Object.freeze(['--ast', 'generate an abstract syntax tree of nodes']), Object.freeze(['-b', '--bare', 'compile without a top-level function wrapper']), Object.freeze(['-c', '--compile', 'compile to JavaScript and save as .js files']), Object.freeze(['-e', '--eval', 'pass a string from the command line as input']), Object.freeze(['-h', '--help', 'display this help message']), Object.freeze(['-i', '--interactive', 'run an interactive CoffeeScript REPL']), Object.freeze(['-j', '--join [FILE]', 'concatenate the source CoffeeScript before compiling']), Object.freeze(['-l', '--literate', 'treat stdio as literate style coffeescript']), Object.freeze(['-m', '--map', 'generate source map and save as .js.map files']), Object.freeze(['-M', '--inline-map', 'generate source map and include it directly in output']), Object.freeze(['-n', '--nodes', 'print out the parse tree that the parser produces']), Object.freeze(['--nodejs [ARGS]', 'pass options directly to the "node" binary']), Object.freeze(['--no-header', 'suppress the "Generated by" header']), Object.freeze(['-o', '--output [PATH]', 'set the output path or path/filename for compiled JavaScript']), Object.freeze(['-p', '--print', 'print out the compiled JavaScript']), Object.freeze(['-r', '--require [MODULE*]', 'require the given module before eval or REPL']), Object.freeze(['-s', '--stdio', 'listen for and compile scripts over stdio']), Object.freeze(['-t', '--transpile', 'pipe generated JavaScript through Babel']), Object.freeze(['--tokens', 'print out the tokens that the lexer/rewriter produce']), Object.freeze(['-v', '--version', 'display the version number']), Object.freeze(['-w', '--watch', 'watch scripts for changes and rerun commands'])]);

  let opts = Object.freeze({});

  let sources = Object.freeze([]);

  let sourceCode = Object.freeze([]);

  let notSources = Object.freeze({});

  let watchedDirs = Object.freeze({});
...
'--' will be removed from the argument list.`);
      ...
      opts = Object.freeze({...opts, arguments: [opts.arguments[0]].concat(opts.arguments.slice(2))});
    }
    ...
    if (opts.require) {
      opts = Object.freeze({...opts, prelude: makePrelude(opts.require)});
    }
    ...
    if (opts.output) {
      ...
      if (indexOf.call(outputBasename, '.') >= 0 && (outputBasename !== '.' && outputBasename !== '..') && !helpers.ends(opts.output, path.sep)) {
        opts = Object.freeze({...opts, outputFilename: outputBasename});
        opts = Object.freeze({...opts, outputPath: path.resolve(path.dirname(opts.output))});
      } else {
        opts = Object.freeze({...opts, outputFilename: null});
        opts = Object.freeze({...opts, outputPath: path.resolve(opts.output)});
      }
    }
    if (opts.join) {
      opts = Object.freeze({...opts, join: path.resolve(opts.join)});
      console.error(`
...
`);
  ...
  const compilePath = function(source, topLevel, base) {
    ...
    if (stats.isDirectory()) {
      if (path.basename(source) === 'node_modules') {
        notSources = Object.freeze({...notSources, [source]: true});
        return;
      }
      if (opts.run) {
      ...
      sourceCode = Object.freeze([...sourceCode, null]);
      notSources = Object.freeze({...notSources, [source]: false});
      if (opts.watch) {
        watch(source, base);
      }
      try {
      ...
      return notSources = Object.freeze({...notSources, [source]: true});
    }
  };
  ...
  const compileScript = function(file, input, base = null) {
    ...
    try {
      ...
      if (opts.tokens) {
        ...
        sourceCode = Object.freeze({...sourceCode, [sources.indexOf(task.file)]: task.input});
        return compileJoin();
      } else {
        compiled = CoffeeScript.compile(task.input, task.options);
  ...
  const watchDir = function(source, base) {
    ...
    watchedDirs = Object.freeze({...watchedDirs, [source]: true});
    try {
      return startWatcher();
    } catch (error) {
      err = error;
  ...
  const removeSourceDir = function(source, base) {
    ...
    watchedDirs = Object.freeze({...watchedDirs, [source]: false});
  ...
  const removeSource = function(source, base) {
    ...
    sources = Object.freeze([...sources.slice(0, index), ...sources.slice(index + 1)]);
    sourceCode = Object.freeze([...sourceCode.slice(0, index), ...sourceCode.slice(index + 1)]);
    if (!opts.join) {
...
See https://coffeescript.org/#transpilation`);
      ...
      if (typeof opts.transpile !== 'object') {
        opts = Object.freeze({...opts, transpile: {}});
      }
      opts = Object.freeze({...opts, transpile: CoffeeScript.transpile});
      ...
      if (!opts.transpile.filename) {
        opts = Object.freeze({...opts, filename: filename || path.resolve(base || process.cwd(), '<anonymous>')});
      }
    } else {
      opts = Object.freeze({...opts, transpile: false});
    }
    answer = {
      filename: filename,
