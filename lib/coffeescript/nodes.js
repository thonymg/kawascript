...
let Access, Arr, Assign, AwaitReturn, Base, BindingPattern, Block, BooleanLiteral, Call, Catch, Class, ClassProperty, ClassPrototypeProperty, Code, CodeFragment, ComputedPropertyName, DefaultLiteral, Directive, DynamicImport, DynamicImportCall, Elision, EmptyInterpolation, ExecutableClassBody, Existence, Expansion, ExportAllDeclaration, ExportDeclaration, ExportDefaultDeclaration, ExportNamedDeclaration, ExportSpecifier, ExportSpecifierList, Extends, For, FuncDirectiveReturn, FuncGlyph, HereComment, HoistTarget, IdentifierLiteral, If, ImportClause, ImportDeclaration, ImportDefaultSpecifier, ImportNamespaceSpecifier, ImportSpecifier, ImportSpecifierList, In, Index, InfinityLiteral, Interpolation, JSXAttribute, JSXAttributes, JSXElement, JSXEmptyExpression, JSXExpressionContainer, JSXIdentifier, JSXNamespacedName, JSXTag, JSXText, JS_FORBIDDEN, LineComment, Literal, LiteralPattern, MatchArm, MatchNode, MetaProperty, ModuleDeclaration, ModuleSpecifier, ModuleSpecifierList, NaNLiteral, NullLiteral, NumberLiteral, Obj, ObjectProperty, Op, OrPattern, Param, Parens, PassthroughLiteral, PropertyName, Range, RegexLiteral, RegexWithInterpolations, Return, Root, Scope, Sequence, Slice, Splat, StatementLiteral, StringLiteral, StringWithInterpolations, Super, SuperCall, Switch, SwitchWhen, TaggedTemplateCall, TemplateElement, ThisLiteral, Throw, Try, UndefinedLiteral, Value, While, YieldReturn, addDataToNode, attachCommentsToNode, compact, del, ends, extend, flatten, isFunction, isNumber, isPlainObject, isUnassignable, jisonLocationDataToAstLocationData, locationDataToString, merge, mergeAstLocationData, mergeLocationData, parseNumber, replaceUnicodeCodePointEscapes, some, starts, throwSyntaxError,
  indexOf = [].indexOf,
  splice = [].splice,
  slice1 = [].slice;
...
exports.MatchArm = MatchArm = (function() {
  class MatchArm extends Base {
    ...
    compileArm(o, subjectFrags, isFirst) {
      let condFrags, guardFrags, idt, idt2, o2, opener;
      idt = o.indent;
      idt2 = idt + TAB;
      o2 = merge(o, {
        indent: idt2
      ...
      if (this.guard) {
        ...
        // Substitute pattern binding names with subject variable in guard fragments.
        // e.g. `| n if n > 0 ->` compiles the guard as `m > 0` (not `n > 0`).
        const bindings = this.pattern.bindings();
        if (bindings.length > 0) {
          const subjectCode = subjectFrags.map(f => f.code).join('');
          guardFrags = guardFrags.map(f => {
            if (!f.code) return f;
            let code = f.code;
            for (const bname of bindings) {
              code = code.replace(new RegExp(`\\b${bname}\\b`, 'g'), subjectCode);
            }
            return Object.assign(Object.create(Object.getPrototypeOf(f)), f, {code});
          });
        }
    ...
    injectBindings(o, subjectFrags) {
      let fragments, j, len1, name, ref1;
      fragments = [];
...
exports.MatchNode = MatchNode = (function() {
  class MatchNode extends Base {
    ...
    compileNode(o) {
      ...
      tempName = o.scope.freeVariable('m', {reserve: false});
      idt = this.tab;
...
// PipeOp — `a |> f b, c` compiles to `f(a, b, c)` (data-first, like Elixir).
exports.PipeOp = class PipeOp extends Base {
  ...
  compilePipeIntoCall(o, leftFrags, call) {
    const fnFrags = call.variable.compileToFragments(o, LEVEL_ACCESS);
    let argFrags = [].concat(leftFrags);
...
exports.PipeOp.prototype.children = ['left', 'right'];

exports.PipeOp.prototype.isStatement = NO;
