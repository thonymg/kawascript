...
(function() {
  ...
  const BALANCED_PAIRS = Object.freeze([Object.freeze(['(', ')']), Object.freeze(['[', ']']), Object.freeze(['{', '}']), Object.freeze(['INDENT', 'OUTDENT']), Object.freeze(['CALL_START', 'CALL_END']), Object.freeze(['PARAM_START', 'PARAM_END']), Object.freeze(['INDEX_START', 'INDEX_END']), Object.freeze(['STRING_START', 'STRING_END']), Object.freeze(['INTERPOLATION_START', 'INTERPOLATION_END']), Object.freeze(['REGEX_START', 'REGEX_END'])]);

  let INVERSES = Object.freeze({});

  let EXPRESSION_START = Object.freeze([]);

  let EXPRESSION_END = Object.freeze([]);
  ...
  const EXPRESSION_CLOSE = Object.freeze(['CATCH', 'THEN', 'ELSE', 'FINALLY']).concat(EXPRESSION_END);

  // Tokens that, if followed by an `IMPLICIT_CALL`, indicate a function invocation.
  const IMPLICIT_FUNC = Object.freeze(['IDENTIFIER', 'PROPERTY', 'SUPER', ')', 'CALL_END', ']', 'INDEX_END', '@', 'THIS']);

  // If preceded by an `IMPLICIT_FUNC`, indicates a function invocation.
  const IMPLICIT_CALL = Object.freeze(['IDENTIFIER', 'JSX_TAG', 'PROPERTY', 'NUMBER', 'INFINITY', 'NAN', 'STRING', 'STRING_START', 'REGEX', 'REGEX_START', 'JS', 'NEW', 'PARAM_START', 'CLASS', 'IF', 'TRY', 'SWITCH', 'THIS', 'DYNAMIC_IMPORT', 'IMPORT_META', 'NEW_TARGET', 'UNDEFINED', 'NULL', 'BOOL', 'UNARY', 'DO', 'DO_IIFE', 'YIELD', 'AWAIT', 'UNARY_MATH', 'SUPER', 'THROW', '@', '->', '=>', '[', '(', '{', '--', '++']);

  const IMPLICIT_UNSPACED_CALL = Object.freeze(['+', '-']);

  // Tokens that always mark the end of an implicit call for single-liners.
  const IMPLICIT_END = Object.freeze(['POST_IF', 'FOR', 'WHILE', 'UNTIL', 'WHEN', 'BY', 'LOOP', 'TERMINATOR']);

  // Single-line flavors of block expressions that have unclosed endings.
  // The grammar can’t disambiguate them, so we insert the implicit indentation.
  const SINGLE_LINERS = Object.freeze(['ELSE', '->', '=>', 'TRY', 'FINALLY', 'THEN']);

  const SINGLE_CLOSERS = Object.freeze(['TERMINATOR', 'CATCH', 'FINALLY', 'ELSE', 'OUTDENT', 'LEADING_WHEN']);

  // Tokens that end a line.
  const LINEBREAKS = Object.freeze(['TERMINATOR', 'INDENT', 'OUTDENT']);

  // Tokens that close open calls when they follow a newline.
  const CALL_CLOSERS = Object.freeze(['.', '?.', '::', '?::', 'PIPE']);

  // Tokens that prevent a subsequent indent from ending implicit calls/objects
  const CONTROL_IN_IMPLICIT = Object.freeze(['IF', 'TRY', 'FINALLY', 'CATCH', 'CLASS', 'SWITCH']);
