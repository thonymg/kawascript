...
(function() {
  ...
  const JS_KEYWORDS = Object.freeze(['true', 'false', 'null', 'this', 'new', 'delete', 'typeof', 'in', 'instanceof', 'return', 'throw', 'break', 'continue', 'debugger', 'yield', 'await', 'if', 'else', 'switch', 'for', 'while', 'do', 'try', 'catch', 'finally', 'class', 'extends', 'super', 'import', 'export', 'default']);

  let COFFEE_KEYWORDS = Object.freeze(['undefined', 'Infinity', 'NaN', 'then', 'unless', 'until', 'loop', 'of', 'by', 'when', 'let', 'match']);
  ...
  const RESERVED = Object.freeze(['case', 'function', 'var', 'void', 'with', 'const', 'enum', 'native', 'implements', 'interface', 'package', 'private', 'protected', 'public', 'static']);

  const STRICT_PROSCRIBED = Object.freeze(['arguments', 'eval']);
  ...
  const COMPOUND_ASSIGN = Object.freeze(['-=', '+=', '/=', '*=', '%=', '||=', '&&=', '?=', '<<=', '>>=', '>>>=', '&=', '^=', '|=', '**=', '//=', '%%=']);

  // Unary tokens.
  const UNARY = Object.freeze(['NEW', 'TYPEOF', 'DELETE']);
  ...
  const RELATION = Object.freeze(['IN', 'OF', 'INSTANCEOF']);

  // Boolean tokens.
  const BOOL = Object.freeze(['TRUE', 'FALSE']);

  // Tokens which could legitimately be invoked or indexed. An opening
  // parentheses or bracket following these tokens will be recorded as the start
  // of a function invocation or indexing operation.
  const CALLABLE = Object.freeze(['IDENTIFIER', 'PROPERTY', ')', ']', '?', '@', 'THIS', 'SUPER', 'DYNAMIC_IMPORT']);

  const INDEXABLE = CALLABLE.concat(Object.freeze(['NUMBER', 'INFINITY', 'NAN', 'STRING', 'STRING_END', 'REGEX', 'REGEX_END', 'BOOL', 'NULL', 'UNDEFINED', '}', '::']));

  // Tokens which can be the left-hand side of a less-than comparison, i.e. `a<b`.
  const COMPARABLE_LEFT_SIDE = Object.freeze(['IDENTIFIER', ')', ']', 'NUMBER']);

  // Tokens which a regular expression will never immediately follow (except spaced
  // CALLABLEs in some cases), but which a division operator can.

  // See: http://www-archive.mozilla.org/js/language/js20-2002-04/rationale/syntax.html#regular-expressions
  const NOT_REGEX = INDEXABLE.concat(Object.freeze(['++', '--']));

  // Tokens that, when immediately preceding a `WHEN`, indicate that the `WHEN`
  // occurs at the start of a line. We disambiguate these from trailing whens to
  // avoid an ambiguity in the grammar.
  const LINE_BREAK = Object.freeze(['INDENT', 'OUTDENT', 'TERMINATOR']);

  // Additional indent in front of these is ignored.
  const INDENTABLE_CLOSERS = Object.freeze([')', '}', ']']);
