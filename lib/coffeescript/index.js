...
(function() {
  ...
  const transpile = function(js, options) {
    let babel;
    try {
  ...
  const compile = function(code, options) {
  ...
  const evalCS = function(code, options = {}) {
    ...
    o.replMode = true; // use var instead of const/let to prevent redeclaration across eval calls
    js = compile(code, o);
    if (sandbox === global) {
  ...
  const _compileRawFileContent = function(raw, filename, options = {}) {
  ...
  const _compileFile = function(filename, options = {}) {
    let raw;
    raw = fs.readFileSync(filename, 'utf8');
    return _compileRawFileContent(raw, filename, options);
  };
